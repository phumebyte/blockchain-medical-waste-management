...
app.post('/mintNFT', async (req, res) => {
    ...
    try {
        ...
        const metadata = {
            ...
            image: 'https://example.com/waste-image.jpg',  // Add a placeholder image or dynamic image
            attributes: [
        ...
        waste.blockchainMetadata = metadata.attributes;
        await waste.save();
...
app.get('/nftTransactions/:nftId', async (req, res) => {
    const { nftId } = req.params;

    try {
        const response = await axios.get(`https://api.verbwire.com/v1/nft/transaction/${nftId}`, {
            headers: {
                'Authorization': `Bearer ${process.env.VERBWIRE_API_KEY}`
            }
        });
